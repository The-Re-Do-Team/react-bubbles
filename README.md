...
- [ ] Explain what a token is used for. Answer: A token is saved in localStorage for client-side authentication.
- [ ] What steps can you take in your web apps to keep your data secure? Answer: Use authorization tokens for authentication, use protected routes.
- [ ] Describe how web servers work. Answer: A web server is a computer that stores code to serve or host webpages and apps.
- [ ] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers. Answer: POST, GET, PUT, DELETE, PATCH
